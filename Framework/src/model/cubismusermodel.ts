...
export class PndyTimerThingForExpressions {
  
  public constructor(iduration: number, iexp: string[]) {
    this._max_timer = iduration;
    this._temp_timer = 0.0;
    this._enabled = false;
    this._expression = iexp;
  }
  public setEnable(ib: boolean) {
    if (this._enabled==false){// && ib==true) {
      //  trueに切り替えるときに temp_timerクリア
      this._temp_timer = 0.0;
    }
    this._enabled = ib;
  }
  public update(deltaTimeSeconds: number): string {
    if (this._enabled==true) {
      if (this._temp_timer>this._max_timer) {
        let ind = Math.floor(Math.random() * this._expression.length);
        return this._expression[ind];
      }
      else {
        this._temp_timer += deltaTimeSeconds;
      }
    }
  ...
  private _max_timer : number;
  private _temp_timer: number;
...
export class CubismUserModel {
  ...
  public constructor() {
    ...
    //  pndy
    let exp_face_to_normal = ["HUTSUU"];
    this._timer_face_to_normal = new PndyTimerThingForExpressions(32.0, exp_face_to_normal);
    
    let exp_neko_sigusa = ["KUUCHUU_0", "KUUCHUU_1", "KUUCHUU_2", "KUUCHUU_3"];
    this._timer_neko_sigusa = new PndyTimerThingForExpressions(8.0, exp_neko_sigusa);
    //  初期からキョロキョロさせる
    this._timer_neko_sigusa.setEnable(true);

    this._expressionMap = new Map<string, string>();
    this._expressionMap.set("嬉しい",   "KIRAKIRA");
    this._expressionMap.set("普通",     "HUTSUU");
    this._expressionMap.set("怒り",     "AOZM");
    this._expressionMap.set("哀しみ",   "CRY");
    this._expressionMap.set("落着き",   "HUTSUU");
  ...
  //  pndy タイマー表情
  protected _timer_face_to_normal: PndyTimerThingForExpressions;
  protected _timer_neko_sigusa: PndyTimerThingForExpressions;
  protected _expressionMap: Map<string, string>;
}
