...
export class LAppModel extends CubismUserModel {
  ...
  public update_pndy_neko(deltaTimeSeconds: number): void {

    //  しゃべった後の顔をもとに戻す
    let back_to_idle = this._timer_face_to_normal.update(deltaTimeSeconds);
    if (back_to_idle!="") {

      console.log("GOING BACK TO NORMAL FACE");

      this.setExpression(back_to_idle);

      this._timer_face_to_normal.setEnable(false);
      this._timer_neko_sigusa.setEnable(true);
    }

    //  猫しぐさのチェック
    let neko_sigusa = this._timer_neko_sigusa.update(deltaTimeSeconds);
    if (neko_sigusa!="") {

      console.log("NEKO_SIGUSA = "+neko_sigusa);

      this.setExpression(neko_sigusa);
      this._timer_neko_sigusa.setEnable(false);
      this._timer_face_to_normal.setEnable(true);
    }
  }
  ...
  public update(): void {
    ...
    //  pndy
    this.update_pndy_neko(deltaTimeSeconds);
  ...
  public async startVoice() {
    ...
    //  http://machiko.f5.si:443
    let api_server : string = "http://127.0.0.1:443"
    //let api_server : string = "http://machiko.f5.si:443"//

    // ctrl+enterでもsubmit受付
    ...
    // submit処理 内容取り出してrequestMessageする
    const clickButton = () => {
      const inputText = getInputText()
      if (inputText === "" || !inputText) return

      //  requestMessageから帰ってきたら ファイル名からデータストリーム開く
      requestMessage(inputText)
        .then((fileName) => {
          console.log(fileName)
          const path = `${api_server}/audio?file_name=${fileName}`
    ...
    const requestMessage = async (message:string) => {
      const url = `${api_server}/getResponse`; 
      ...
      //  返ってきたばかりのjson["emotion"] にエモーションが入っている
      //  これでexpression変更
      let expression_choise = this._expressionMap.get(json["emotion"]);
      this.setExpression(expression_choise);
      ...
      //  返事を表示
      let resmsg_content = document.getElementById('resmsg') as HTMLTextAreaElement;
      resmsg_content.value = json["resmsg"];
